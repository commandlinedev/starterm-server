...
//!
//! ```
//! use starterm::{Filter, http::Response};
//!
//! // Returns an empty `200 OK` response.
//! let empty_200 = starterm::any().map(starterm::reply);
//!
//! // Returns a `200 OK` response with custom header and body.
//! let custom = starterm::any().map(|| {
//!     Response::builder()
...
//! let routes = starterm::get().and(empty_200)
//!     .or(starterm::post().and(custom));
//! ```
...
/// let route = starterm::path("just-ok")
///     .map(|| {
...
/// // `[1, 3, 7, 13]`
/// let route = starterm::path("ids")
///     .map(|| {
///         let our_ids = vec![1, 3, 7, 13];
///         starterm::reply::json(&our_ids)
///     });
/// ```
///
/// # Note
...
/// <html>
///     <head>
///         <title>HTML with starterm!</title>
///     </head>
///     <body>
///         <h1>starterm + HTML = &hearts;</h1>
///     </body>
/// </html>
/// "#;
///
...
/// }
///
/// impl starterm::Reply for Message {
///     fn into_response(self) -> starterm::reply::Response {
...
/// }
///
/// let route = starterm::any().map(handler);
/// ```
...
///     .map(starterm::reply)
///     .map(|reply| {
///         starterm::reply::with_status(reply, starterm::http::StatusCode::CREATED)
///     });
/// ```
...
/// Wrap an `impl Reply` to change its `StatusCode`.
///
/// Returned by `starterm::reply::with_status`.
#[derive(Debug)]
...
///     .map(starterm::reply)
///     .map(|reply| {
///         starterm::reply::with_header(reply, "server", "starterm")
///     });
/// ```
...
///
/// Returned by `starterm::reply::with_header`.
#[derive(Debug)]
