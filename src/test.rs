...
//! use starterm::Filter;
//!
//! fn sum() -> impl Filter<Extract = (u32,), Error = starterm::Rejection> + Copy {
//!     starterm::path::param()
...
//! }
//!
//! fn math() -> impl Filter<Extract = (String,), Error = starterm::Rejection> + Copy {
//!     starterm::post()
//!         .and(sum())
//!         .map(|z: u32| {
//!             format!("Sum = {}", z)
...
//! #[tokio::test]
//! async fn test_sum() {
//! #    let sum = || starterm::any().map(|| 3);
//!     let filter = sum();
//!
//!     // Execute `sum` and get the `Extract` back.
//!     let value = starterm::test::request()
//!         .path("/1/2")
//!         .filter(&filter)
//!         .await
//!         .unwrap();
...
//! #[test]
//! fn test_math() {
//! #    let math = || starterm::any().map(starterm::reply);
//!     let filter = math();
//!
