...
///
/// ```
/// use starterm::{Filter, filters::BoxedFilter, Reply};
///
...
/// ```
///
type BoxedFilterInner<T> = dyn Filter<
        Extract = T,
        Error = Rejection,
...
/// A filter that has been boxed to hide its concrete type.
pub struct BoxedFilter<T: Tuple> {
    filter: Arc<BoxedFilterInner<T>>,
}
