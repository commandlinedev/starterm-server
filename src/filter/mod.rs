...
pub trait Filter: FilterBase {
    ...
    /// // Match `/hello/:name`...
    /// starterm::path("hello")
    ///     .and(starterm::path::param::<String>());
    /// ```
    ...
    /// starterm::path::param::<u32>()
    ///     .or(starterm::path::param::<SocketAddr>());
    /// ```
    ...
    ///
    /// // Map `/:id`
    /// starterm::path::param().map(|id: u64| {
    ///   format!("Hello #{}", id)
    /// });
    /// ```
    ///
    ...
    ///
    /// // Map `/:id`
    /// starterm::path::param().then(|id: u64| async move {
    ///   format!("Hello #{}", id)
    /// });
    /// ```
    ...
    ///
    /// // Validate after `/:id`
    /// starterm::path::param().and_then(|id: u64| async move {
    ///     if id != 0 {
    ///         Ok(format!("Hello #{}", id))
    ///     } else {
    ///         Err(starterm::reject::not_found())
    ///     }
    /// });
    /// ```
    ...
    ///
    /// [ex]: https://github.com/commandlinedev/starterm-server/blob/master/examples/rejections.rs
    ...
    /// use starterm::Filter;
    ///
    /// let client_ip = starterm::header("x-real-ip")
    ///     .or(starterm::header("x-forwarded-for"))
    ///     .unify()
    ///     .map(|ip: SocketAddr| {
    ...
    /// but just `()`, since starterm will wrap it up into a `((),)`.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// let route = route.with(starterm::log("example"));
    /// ```
    ...
    /// use starterm::Filter;
    ///
    /// fn impl_reply() -> starterm::filters::BoxedFilter<(impl starterm::Reply,)> {
    ///     starterm::any()
    ///         .map(starterm::reply)
    ///         .boxed()
    /// }
    ///
    /// fn named_i32() -> starterm::filters::BoxedFilter<(i32,)> {
    ///     starterm::path::param::<i32>()
    ///         .boxed()
    /// }
    ///
    /// fn named_and() -> starterm::filters::BoxedFilter<(i32, String)> {
    ///     starterm::path::param::<i32>()
    ///         .and(starterm::header::<String>("host"))
    ///         .boxed()
    /// }
    /// ```
