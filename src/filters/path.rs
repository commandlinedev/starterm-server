...
//! # Routing
//!
//! Routing in starterm is simple yet powerful.
//!
...
//! ```
//! # use starterm::Filter;
//! // GET /hello/from/starterm
//! let hello_from_starterm = starterm::path!("hello" / "from" / "starterm").map(|| {
//!     "Hello from starterm!"
//! });
//! ```
//!
...
//! # use starterm::Filter;
//! // GET /sum/:u32/:u32
//! let sum = starterm::path!("sum" / u32 / u32).map(|a, b| {
//!     format!("{} + {} = {}", a, b, a + b)
//! });
//! ```
//!
...
//! # use starterm::Filter;
//! // GET /:u16/times/:u16
//! let times = starterm::path!(u16 / "times" / u16).map(|a, b| {
...
//! ```
//! # use starterm::Filter;
//! # let sum = starterm::any().map(starterm::reply);
//! # let times = sum.clone();
//! // GET /math/sum/:u32/:u32
//! // GET /math/:u16/times/:u16
//! let math = starterm::path("math");
...
//! ```
//! # use starterm::Filter;
//! # let sum = starterm::path("sum");
//! # let times = starterm::path("times");
//! // GET /math/sum/:u32/:u32
//! // GET /math/:u16/times/:u16
//! let math = starterm::path("math")
//!     .and(sum.or(times));
...
//! ```
//! # use starterm::Filter;
//! # let hi = starterm::path("hi");
//! # let hello_from_starterm = hi.clone();
//! # let bye = hi.clone();
//! # let math = hi.clone();
//! // GET /hi
//! // GET /hello/from/starterm
...
/// use starterm::Filter;
///
/// let route = starterm::path("foo")
///     .and(starterm::path::tail())
///     .map(|tail| {
...
///
/// ```
/// use starterm::{Filter, path::FullPath};
/// use std::{collections::HashMap, sync::{Arc, Mutex}};
///
/// let counts = Arc::new(Mutex::new(HashMap::new()));
/// let access_counter = starterm::path::full()
...
///
/// // Match `/sum/:a/:b`
/// let route = starterm::path!("sum" / u32 / u32)
///     .map(|a, b| {
///         format!("{} + {} = {}", a, b, a + b)
///     });
/// ```
...
/// use starterm::Filter;
///
/// let prefix = starterm::path!("math" / "sum" / ..);
///
/// let sum = starterm::path!(u32 / u32)
///     .map(|a, b| {
///         format!("{} + {} = {}", a, b, a + b)
///     });
///
/// let help = starterm::path::end()
