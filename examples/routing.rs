...
async fn main() {
    ...
    // GET /
    let hello_world = starterm::path::end().map(|| "Hello, World at root!");

    // GET /hi
    let hi = starterm::path("hi").map(|| "Hello, World!");

    // How about multiple segments? First, we could use the `path!` macro:
    //
    // GET /hello/from/starterm
    let hello_from_starterm =
        starterm::path!("hello" / "from" / "starterm").map(|| "Hello from starterm!");

    // Fine, but how do I handle parameters in paths?
    //
    // GET /sum/:u32/:u32
    let sum = starterm::path!("sum" / u32 / u32).map(|a, b| format!("{} + {} = {}", a, b, a + b));

    // Any type that implements FromStr can be used, and in any order:
    //
    // GET /:u16/times/:u16
    let times =
        starterm::path!(u16 / "times" / u16).map(|a, b| format!("{} times {} = {}", a, b, a * b));
    ...
    // GET /math/sum/:u32/:u32
    // GET /math/:u16/times/:u16
    let math = starterm::path("math").and(sum.or(times));

    // We can use the end() filter to match a shorter path
    let help = starterm::path("math")
