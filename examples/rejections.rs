...
use starterm::{reject, Filter, Rejection, Reply};
...
#[tokio::main]
async fn main() {
    let math = starterm::path!("math" / u16);
    let div_with_header =
        math.and(starterm::get())
            .and(div_by())
            .map(|num: u16, denom: NonZeroU16| {
                starterm::reply::json(&Math {
                    op: format!("{} / {}", num, denom),
                    output: num / denom.get(),
                })
            });

    let div_with_body = math.and(starterm::post()).and(starterm::body::json()).map(
        |num: u16, body: DenomRequest| {
            starterm::reply::json(&Math {
...
fn div_by() -> impl Filter<Extract = (NonZeroU16,), Error = Rejection> + Copy {
    starterm::header::<u16>("div-by").and_then(|n: u16| async move {
...
async fn handle_rejection(err: Rejection) -> Result<impl Reply, Infallible> {
    ...
    if err.is_not_found() {
    ...
    } else if let Some(e) = err.find::<starterm::filters::body::BodyDeserializeError>() {
        ...
        code = StatusCode::BAD_REQUEST;
    } else if err.find::<starterm::reject::MethodNotAllowed>().is_some() {
    ...
    let json = starterm::reply::json(&ErrorMessage {
        code: code.as_u16(),
        message: message.into(),
    });

    Ok(starterm::reply::with_status(json, code))
}
