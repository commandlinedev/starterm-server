...
async fn main() {
    ...
    let routes = api.with(starterm::log("todos"));
    // Start up the server...
...
mod filters {
    ...
    pub fn todos(
        db: Db,
    ) -> impl Filter<Extract = (impl starterm::Reply,), Error = starterm::Rejection> + Clone {
        todos_list(db.clone())
    ...
    pub fn todos_list(
        ...
        starterm::path!("todos")
            .and(starterm::get())
            .and(starterm::query::<ListOptions>())
            .and(with_db(db))
    ...
    pub fn todos_delete(
        ...
        let admin_only = starterm::header::exact("authorization", "Bearer admin");
    ...
    fn with_db(db: Db) -> impl Filter<Extract = (Db,), Error = std::convert::Infallible> + Clone {
        starterm::any().map(move || db.clone())
    }

    fn json_body() -> impl Filter<Extract = (Todo,), Error = starterm::Rejection> + Clone {
        // When accepting a body, we want a JSON body
        // (and to reject huge payloads)...
        starterm::body::content_length_limit(1024 * 16).and(starterm::body::json())
    }
}
...
mod handlers {
    use super::models::{Db, ListOptions, Todo};
    use starterm::http::StatusCode;
    use std::convert::Infallible;

    pub async fn list_todos(opts: ListOptions, db: Db) -> Result<impl starterm::Reply, Infallible> {
        ...
        let todos: Vec<Todo> = todos
            ...
            .collect();
        Ok(starterm::reply::json(&todos))
    }

    pub async fn create_todo(create: Todo, db: Db) -> Result<impl starterm::Reply, Infallible> {
    ...
    pub async fn update_todo(
        ...
        update: Todo,
        db: Db,
    ) -> Result<impl starterm::Reply, Infallible> {
    ...
    pub async fn delete_todo(id: u64, db: Db) -> Result<impl starterm::Reply, Infallible> {
