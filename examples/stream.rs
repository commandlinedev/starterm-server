...
use starterm::{reply::Response, Filter, Reply};
...
async fn main() {
    ...
    let route = starterm::body::content_length_limit(65536)
        .and(starterm::body::stream())
        .then(handler);
    starterm::serve(route).run(([127, 0, 0, 1], 3030)).await;
}

async fn handler(
    mut body: impl Stream<Item = Result<impl Buf, starterm::Error>> + Unpin + Send + Sync,
) -> Response {
