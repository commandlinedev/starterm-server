...
async fn file() {
    ...
    let file = starterm::fs::file("README.md");
...
async fn dir() {
    ...
    let file = starterm::fs::dir("examples");

    let req = starterm::test::request().path("/todos.rs");
    let res = req.reply(&file).await;
    ...
    let malformed_req = starterm::test::request().path("todos.rs");
...
async fn dir_encoded() {
    ...
    let req = starterm::test::request().path("/todos%2ers");
    let res = req.reply(&file).await;
...
async fn dir_not_found() {
    ...
    let req = starterm::test::request().path("/definitely-not-found");
    let res = req.reply(&file).await;
...
async fn dir_bad_path() {
    ...
    let req = starterm::test::request().path("/../README.md");
    let res = req.reply(&file).await;
...
async fn dir_bad_encoded_path() {
    ...
    let req = starterm::test::request().path("/%2E%2e/README.md");
    let res = req.reply(&file).await;
...
async fn dir_fallback_index_on_dir() {
    ...
    let file = starterm::fs::dir("examples");
    let req = starterm::test::request().path("/dir");
    let res = req.reply(&file).await;
